...
class TelegramUploader:
    ...
    async def upload(self):
        ...
        if await aiopath.isfile(self._path):
            walker = [(ospath.dirname(self._path), [], [ospath.basename(self._path)])]
        else:
            walker = await sync_to_async(walk, self._path)
            
        for dirpath, _, files in natsorted(walker):
        ...
        # Don't call on_upload_complete in streaming mode - it's handled by task_listener
        if getattr(self._listener, 'is_streaming', False):
            LOGGER.info(f"Leech Completed (Streaming): {self._listener.name} - {self._total_files} files")
            return
    ...
    async def _send_media_payload(self, method, **kwargs):
        """Helper to send media via reply (if anchor exists) or send (if first file)."""
        if self._sent_msg:
            return await getattr(self._sent_msg, f"reply_{method}")(**kwargs)
        else:
             # First file upload to up_dest - Use Send
            if "quote" in kwargs:
                del kwargs["quote"]
            client = TgClient.user if self._user_session else TgClient.bot
            kwargs["chat_id"] = self._listener.up_dest
            if self._listener.chat_thread_id:
                kwargs["message_thread_id"] = self._listener.chat_thread_id
            return await getattr(client, f"send_{method}")(**kwargs)
    ...
    async def _upload_file(self, cap_mono, file, o_path, force_document=False):
        ...
        # Check self._sent_msg safely (it might be None during first upload)
        if self._sent_msg and (not hasattr(self._sent_msg, "chat") or self._sent_msg.chat is None):
        ...
        try:
            ...
            # Defensive Check: Verify file exists before processing
            if not await aiopath.exists(self._up_path):
                LOGGER.error(f"File vanished before upload processing: {self._up_path} (Original: {o_path})")
                # Fallback: Check if original path still exists (in case rename failed/reverted)
                if await aiopath.exists(o_path):
                    LOGGER.info(f"Found original file at {o_path}, reverting path")
                    self._up_path = o_path
                else:
                    await self._listener.on_upload_error(
                        f"File not found: {ospath.basename(self._up_path)}"
                    )
                    return
            ...
            try:
                ...
                LOGGER.error(f"Error determining document type for {self._up_path}: {e}")
                # Fallback to document upload if type detection fails
            ...
            if (
                ...
                self._sent_msg = await self._send_media_payload(
                    "document",
