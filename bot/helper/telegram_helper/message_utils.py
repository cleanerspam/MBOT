...
async def update_status_message(sid, force=False):
    ...
    async with task_dict_lock:
        ...
        # Rate Limiting: Sliding Window (2 updates per 10 seconds)
        now = time()
        if "update_history" not in status_dict[sid]:
            status_dict[sid]["update_history"] = []
        
        # Clean old timestamps
        status_dict[sid]["update_history"] = [t for t in status_dict[sid]["update_history"] if now - t < 10]
        
        if not force and len(status_dict[sid]["update_history"]) >= 2:
            return  # Rate limited

        status_dict[sid]["update_history"].append(now)
        status_dict[sid]["time"] = now
...
async def send_status_message(msg, user_id=0, force=False):
    ...
    async with task_dict_lock:
        if sid in status_dict:
            # If not forced, try to update existing message to avoid flood
            if not force:
                async with task_dict_lock:
                    if sid in status_dict:
                        # Release lock before calling update to avoid deadlock (update re-acquires lock)
                        pass
                await update_status_message(sid, force=False)
                return
