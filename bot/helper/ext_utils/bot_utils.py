...
async def get_content_type(url):
    try:
        # Jackett URLs should be treated as magnet/torrent sources
        # They may redirect to magnet: scheme which httpx can't follow
        async with AsyncClient(follow_redirects=True, timeout=10.0) as client:
            response = await client.get(url, follow_redirects=True)
            final_url = str(response.url)
            content_type = response.headers.get("Content-Type")
            
            # Check if response contains HTML with magnet link
            if content_type and "text/html" in content_type and response.status_code == 200:
                from re import search
                from html import unescape
                # Try to extract magnet link from HTML
                magnet_match = search(r'(magnet:\?[^"\'<>\s]+)', response.text)
                if magnet_match:
                    LOGGER.info(f"Extracted magnet link from HTML response")
                    return "application/x-magnet", unescape(magnet_match.group(1))
            
            
            return content_type, final_url
    except Exception as e:
        # Check if it's an unsupported protocol error (redirect to magnet)
        if hasattr(e, "request") and hasattr(e.request, "url"):
             url_str = str(e.request.url)
             # Fix malformed magnet links where httpx adds domain (magnet://domain?...)
             if "magnet://" in url_str and "?" in url_str:
                 url_str = "magnet:?" + url_str.split("?", 1)[1]
             
             if url_str.startswith("magnet:"):
                 LOGGER.info(f"Redirected to magnet link via UnsupportedProtocol exception")
                 return "application/x-magnet", url_str
        
        # Also check string representation for magnet link if attributes missing
        e_str = str(e)
        if "magnet:" in e_str:
             import re
             from html import unescape
             magnet_match = re.search(r'(magnet:\?[^"\'<>\s]+)', e_str)
             if magnet_match:
                 LOGGER.info(f"Extracted magnet link from exception message")
                 return "application/x-magnet", unescape(magnet_match.group(1))

        LOGGER.error(f"get_content_type failed for {url}: {type(e).__name__}: {e}")
        return None, None
