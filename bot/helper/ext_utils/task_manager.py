...
# GoogleDriveSearch module removed - duplicate checking disabled
...
async def stop_duplicate_check(listener):
    # GoogleDriveSearch module no longer exists - duplicate checking disabled
    # Always return False to skip duplicate checking
    return False, None
...
async def limit_checker(listener, yt_playlist=0):
    ...
    # Skip size limits for streaming mode - files processed sequentially
    if listener.is_streaming:
        LOGGER.info("[STREAMING] Skipping Size Limit (streaming mode enabled)")
        return
...
async def pre_task_check(message):
    ...
    if msg:
        if message.from_user:
            ...
            username = message.sender_chat.title if message.sender_chat else "Unknown"
