...
# from .mirror_leech_utils.gdrive_utils.list import GoogleDriveList  # Module removed
...
class TaskConfig:
    def __init__(self):
        ...
        # Per-torrent streaming stats - keyed by torrent hash
        # This prevents state interference between concurrent torrents
        # and allows proper cleanup when torrents complete
        self.streaming_stats = {}  # {torrent_hash: {stats_dict}}
        self.streaming_stats_lock = None  # Will be initialized as asyncio.Lock() in TaskListener
    ...
    def _set_mode_engine(self):
        ...
        out_mode = f"#{'Leech' if self.is_leech else 'RClone' if self.up_dest.startswith('mrcc:') or is_rclone_path(self.up_dest) else 'Mirror'}"
        ...
        in_mode = f"#{'Mega' if self.is_mega else 'qBit' if self.is_qbit else 'RCloneDL' if self.is_rclone else 'ytdlp' if self.is_ytdlp else 'GDrive' if self.is_gdrive else 'Aria2' if (self.source_url and self.source_url != self.message.link) else 'TgMedia'}"
    ...
    async def before_start(self):
        ...
        if self.link not in ["rcl", "gdl"]:
            ...
            # GoogleDriveList module removed - feature disabled
            raise ValueError("Google Drive List feature has been removed")
    ...
    async def get_tag(self, text: list):
        if len(text) > 1 and text[1].startswith("Tag: "):
            ...
            # Try to resolve the peer ID, fallback to message sender if it fails
            try:
                ...
                LOGGER.warning(f"Could not resolve peer ID {id_}: {e}. Using message sender as fallback.")
                # Fallback: use the message sender's user info
                if self.message.from_user:
                    self.user = self.message.from_user
                    self.user_id = self.user.id
                else:
                    # RSS/Channel message - no user, use bot's own ID
