from asyncio import TimeoutError, gather, sleep as asyncio_sleep
...
class TorrentManager:
    ...
    async def initiate(cls):
        if cls.aria2:
            return
        try:
            # Retry connecting to Aria2 with exponential backoff
            # Aria2 needs a few seconds to start after setpkgs.sh runs
            max_retries = 10
            retry_delay = 1
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt < max_retries - 1:
                        LOGGER.info(f"Aria2 not ready yet (attempt {attempt + 1}/{max_retries}), waiting {retry_delay}s...")
                        await asyncio_sleep(retry_delay)
                        retry_delay = min(retry_delay * 2, 15)  # Exponential backoff, max 15s
                    else:
                        raise
            ...
            # Configure qBittorrent preferences for optimal performance
            qb_prefs = {
                "preallocate_all": False,  # Disable pre-allocation for faster start
                
                # SOLUTION 4: Enable DHT and PEX for decentralized peer discovery
                # DHT (Distributed Hash Table) allows finding peers without trackers
                # PEX (Peer Exchange) discovers peers from currently connected peers
                "dht": True,  # Enable DHT for public torrents
                "pex": True,  # Enable PEX (Peer Exchange)
                "lsd": True,  # Enable LSD (Local Service Discovery) for local network peers
            }
            
            await cls.qbittorrent.app.set_preferences(qb_prefs)
            LOGGER.info("qBittorrent configured: pre-allocation disabled, DHT/PEX/LSD enabled")
