...
class Mirror(TaskListener):
    ...
    async def new_event(self):
        ...
        try:
            ...
            LOGGER.error(f"Error in get_tag: {e}", exc_info=True)
        ...
        if (
            ...
            content_type, final_url = await get_content_type(self.link)
            
            # Handle extracted magnet links from HTML
            if content_type == "application/x-magnet" and final_url:
                LOGGER.info(f"Extracted magnet link from Jackett/HTML response")
                self.link = final_url

            # Handle redirects to magnet links or torrent files
            elif final_url and final_url != self.link:
                if is_magnet(final_url):
                    LOGGER.info(f"URL redirected to magnet link")
                    self.link = final_url
                elif final_url.endswith(".torrent") or content_type == "application/x-bittorrent":
                    LOGGER.info(f"URL redirected to torrent file: {final_url}")
                    self.link = final_url
                
                elif content_type is None or re_match(r"text/html|text/plain", content_type):
                    # Still try direct link generator for HTML/text redirects
                    try:
                ...
                # Original logic for non-redirecting URLs
                LOGGER.info(f"[REDIRECT-DEBUG] No redirect detected, trying direct_link_generator")
                try:
                    self.link = await sync_to_async(direct_link_generator, self.link)
                    if isinstance(self.link, tuple):
                        self.link, headers = self.link
