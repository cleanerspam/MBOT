...
async def status_pages(_, query):
    ...
    if data[2] == "ref":
        ...
        # Add streaming upload speeds to total upload speed
        async with task_dict_lock:
            for download in task_dict.values():
                # Check if this is a StreamingStatus instance with active uploaders
                # Check if this is a StreamingStatus instance (or has streaming_stats)
                if hasattr(download, 'streaming_stats'):
                    uploaders = download.streaming_stats.get('uploaders', {})
                    if uploaders:
                        # Sum up speeds from all active uploaders 
                        for uploader in uploaders.values():
                            up_speed += uploader.speed
