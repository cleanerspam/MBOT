...
if UPSTREAM_REPO:
    ...
    if update.returncode == 0:
        # Get commit hash using simple shell command
        commit_hash = "unknown"
        try:
            import subprocess
            result = subprocess.getoutput("git rev-parse --short HEAD")
            if result and not result.startswith("fatal"):
                commit_hash = result.strip()
        except:
            pass
        
        log_info(f"Successfully updated with Latest Updates ! [Commit: {commit_hash}]")
    else:
